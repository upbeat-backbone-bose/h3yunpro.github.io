...
!> 注意：以下示例中，engine实例获取方式，请参考 [H3.IEngine](/docs/cs-instance?id=h3iengine)
...
* 这里只是提醒开发者在用这个事件时要考虑导入触发的情况，但是并不推荐开发者利用这个事件去做导入执行代码的需求，具体原因请看：[为何不推荐利用OnWorkflowInstanceStateChanged事件做导入触发代码的需求](/docs/faq?id=为何不推荐利用OnWorkflowInstanceStateChanged事件做导入触发代码的需求)
