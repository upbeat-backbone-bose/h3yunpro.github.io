...
方法传入参数：
- ```H3.IEngine engine```：总控引擎，文档参考 [H3.IEngine](/docs/cs-instance?id=H3IEngine)

- ```string userId```：查询人的用户Id，一般使用System用户 ```H3.Organization.User.SystemUserId```

- ```H3.DataModel.BizObjectSchema schema```：表单结构实例，文档参考 [H3.DataModel.BizObjectSchema](/docs/cs-instance?id=H3DataModelBizObjectSchema)

- ```H3.DataModel.GetListScopeType getListScopeType```：查询范围，一般使用不限范围的全局查询 ```H3.DataModel.GetListScopeType.GlobalAll```

- ```H3.Data.Filter.Filter filter```：过滤器对象，文档参考 [H3.Data.Filter.Filter](/docs/cs-instance?id=H3DataFilterFilter)
...
- ```H3.IEngine engine```：总控引擎，文档参考 [H3.IEngine](/docs/cs-instance?id=H3.IEngine)
...
!> 注意：
<br/>
数据状态设置为 ```H3.DataModel.BizObjectStatus.Running```，并不会自动创建出流程（创建带流程的表单数据，请参考[如何创建带流程的表单数据](/docs/cs-example?id=如何创建带流程的表单数据)）
<br/><br/>
